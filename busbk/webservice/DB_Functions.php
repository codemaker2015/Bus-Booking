...
class DB_Functions
{
	...
	public function select_person($uname,$password)
	     {
		
	    $qry="select * from tb_person where person_emailid='".$uname."' and  person_password='".$password."'";
...
public function select_stop()
	...
	public function select_conductor()
	{
	    ...
	    //$qry="CALL new_proc('".$uname."','".$password."')";
		$qry="select * from tb_conductor where conductor_un='".$uname."' and conductor_pwd='".$password."'";
	...
	public function select_resid($regno,$dt)
	{
	    //$qry="CALL select_res_id('".$regno."','".$dt."')";
		$qry="SELECT * FROM  view_reserve where res_date='".$dt."' and bus_regno='".$regno."'";
        $result= mysql_query($qry);
		$i=0;
		while($row=mysql_fetch_array($result)){
...
public function searchresult($a,$b,$c)
	...
	public function update_testimonial($id,$text,$na,$val)
	{
		$qry=mysql_query("update tb_testimonials set testimonial_text='$text' person_id = '$na' flag='$val where testimonial_id='$id'");
		$result= mysql_query($qry);
	}
